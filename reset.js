...
(async () => {
  ...
  if (!process.env.DATA_DIR) {
    console.log('CẢNH BÁO: chưa đặt biến DATA_DIR.');
    console.log('  Nếu app đang chạy với DATA_DIR khác, script này sẽ xoá NHẦM thư mục.');
    console.log('  Kiểm tra Environment trên Render trước khi tiếp tục.\n');
  }
  if (!fs.existsSync(DB_FILE)) {
    console.log('Không thấy database ở thư mục trên — có thể app đang lưu ở chỗ khác.');
    console.log('Kiểm tra biến DATA_DIR rồi chạy lại.');
    process.exit(1);
  }

  // Đếm trước để so sánh sau khi xoá
  try {
    const Db0 = require('better-sqlite3');
    const d0 = new Db0(DB_FILE, { readonly: true });
    const n = d0.prepare("SELECT COUNT(*) n FROM users WHERE role='staff'").get().n;
    console.log(`Đang có ${n} nhân viên trong database này.`);
    d0.close();
  } catch (e) {}
  ...
  if (muc.xoa === null) {
    ...
    console.log('');
    console.log('  ⚠ BẮT BUỘC: vào Render bấm Manual Deploy → Restart service.');
    console.log('    Tiến trình đang chạy vẫn giữ database cũ trong bộ nhớ, nên nếu');
    console.log('    không restart thì mở trang vẫn thấy dữ liệu cũ y nguyên.');
    process.exit(0);
  }
  ...
  console.log('Tải lại trang quản trị (Ctrl+Shift+R) là thấy kết quả.');
})();

...
function history(f = {}, scope = null) {
  ...
  // Mặc định của bảng Theo dõi: chỉ hiện việc ĐANG DIỄN RA hoặc CÓ VẤN ĐỀ.
  // Việc đã xong đúng giờ thì không cần chiếm chỗ, xem lại được qua "Tất cả".
  if (f.only === 'active')
    w.push("(a.ended_at IS NULL OR a.is_over_limit=1 OR a.closed_by='auto')");
...
function rollCallByDay(userId, f = {}) {
  ...
  const chiTiet = db.prepare(
    `SELECT r.id, r.day, r.due_at, r.deadline_at, r.answered_at, r.status,
            r.is_makeup, r.defer_reason, r.source, r.photo_path, r.screen_path
     FROM roll_calls r WHERE ${w.join(' AND ')}
     ORDER BY r.day DESC, r.due_at DESC LIMIT 300`).all(...p)
...
/* ============================================================
   ...
   ============================================================ */
/* Soi các ô định danh trong file mà KHÔNG ghi gì vào database.
   Dùng cho nút Xem trước: biết file sai trước khi áp. */
function checkScheduleRows(rows) {
  const out = [];
  for (const r of rows || []) {
    const ten = r.name || r.emp_code || '(không tên)';
    if (r.location && !LOCATIONS.includes(String(r.location).toUpperCase())) {
      out.push(`${ten}: KhuVuc "${r.location}" không hợp lệ. Chỉ nhận ${LOCATIONS.join(' hoặc ')}.`);
    }
    if (r.department && !DEPTS.includes(String(r.department).toUpperCase())) {
      out.push(`${ten}: BoPhan "${r.department}" không có trong hệ thống.`);
    }
    if (r.brand && !BRANDS.includes(String(r.brand).toUpperCase())) {
      out.push(`${ten}: Brand "${r.brand}" không hợp lệ.`);
    }
    if (!Object.keys(r.days || {}).length) {
      out.push(`${ten}: không có ngày làm nào trong tháng này.`);
    }
  }
  return [...new Set(out)];
}
...
function applySchedule(parsed, mode = 'merge', scope = null) {
  ...
  const resolveOne = (r) => {
    ...
    if (!u) {
      ...
      // Báo rõ khi ô trong file không hợp lệ, thay vì lặng lẽ lấy giá trị mặc định.
      // Trước đây KhuVuc sai bị đổi thành VN không nói gì, nên người ở Armenia
      // bị tính giờ Việt Nam và lệch 3 tiếng mà không ai biết.
      if (r.location && !LOCATIONS.includes(String(r.location).toUpperCase())) {
        canhBao.push(`${r.name}: KhuVuc "${r.location}" không hợp lệ, đã đặt tạm là VN. `
          + `Chỉ nhận ${LOCATIONS.join(' hoặc ')}.`);
      }
      if (r.department && !DEPTS.includes(String(r.department).toUpperCase())) {
        canhBao.push(`${r.name}: BoPhan "${r.department}" không có trong hệ thống, đã bỏ trống.`);
      }
      if (r.brand && !BRANDS.includes(String(r.brand).toUpperCase())) {
        canhBao.push(`${r.name}: Brand "${r.brand}" không hợp lệ, đã bỏ trống.`);
      }
  ...
  /* CHỐT AN TOÀN: file không có ngày nào thuộc tháng đang chọn thì DỪNG.
     Nếu không, chế độ Ghi đè sẽ xoá sạch lịch tháng đó rồi chẳng ghi lại gì —
     đúng cái xảy ra khi chọn nhầm tháng áp dụng so với ngày trong file. */
  /* So với THÁNG CỦA DÒNG TIÊU ĐỀ, không so với ngày đã lọc.
     File tháng 9 mà chọn tháng 8 thì chỉ vài cột lọt vào tháng 8 — nếu chỉ nhìn
     phần đã lọc sẽ tưởng đúng 100% rồi xoá sạch lịch tháng 8 thật. */
  const thangTD = parsed.headerMonths || {};
  const tongCot = Object.values(thangTD).reduce((a, b) => a + b, 0);
  const cotDungThang = thangTD[ym] || 0;
  const tyLe = tongCot ? cotDungThang / tongCot : 1;

  if (tongCot && tyLe < 0.6) {
    const xepTheoThang = Object.entries(thangTD).sort((a, b) => b[1] - a[1])
      .map(([m, n]) => `${m} (${n} cột)`);
    const trongThang = cotDungThang, moiNgay = { length: tongCot };
    return {
      ym, mode, matched: [], created: [], skipped: [], outside: [], keySet: [], dayCount: 0,
      aborted: true, warnings: [],
      errors: [`Chỉ ${trongThang}/${moiNgay.length} ngày trong file thuộc tháng ${ym}. `
        + (xepTheoThang.length ? `File chủ yếu là lịch của ${xepTheoThang.slice(0, 3).join(', ')}. ` : '')
        + `Đổi ô "Tháng áp dụng" cho khớp rồi nhập lại. `
        + `KHÔNG xoá hay ghi gì cả — lịch cũ vẫn nguyên.`],
    };
  }
  ...
  db.transaction(() => {
    ...
    for (const { user, rec } of resolved) {
      ...
      if (rec.location && !LOCATIONS.includes(String(rec.location).toUpperCase())) {
        canhBao.push(`${user.name}: KhuVuc "${rec.location}" không hợp lệ, giữ nguyên ${user.location}.`);
      }
      if (rec.department && !DEPTS.includes(String(rec.department).toUpperCase())) {
        canhBao.push(`${user.name}: BoPhan "${rec.department}" không có trong hệ thống, giữ nguyên ${user.department || '(trống)'}.`);
      }
  ...
  const canhBaoGon = [...new Set(canhBao)];
  return { ym, mode, matched, created, skipped, outside, keySet, dayCount,
           warnings: canhBaoGon,
           errors: [...parsed.errors, ...canhBaoGon] };
}

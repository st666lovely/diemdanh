...
app.get('/api/admin/board', requireUser, requireAdmin, (req, res) => {
  ...
  res.json({
    ...
    scope: req.scope,
    shifts: D.shiftOptions(req.scope),
...
/* --- Độ dài ca của từng người ---
   File lịch chỉ ghi một mốc giờ vào ca; giờ kết = mốc đó + độ dài ca. */
app.put('/api/admin/users/:id/shift-hours', requireUser, requireAdmin, (req, res) => {
  const u = targetUser(req, res); if (!u) return;
  const r = D.setShiftHours(u.id, (req.body || {}).hours);
  if (r.ok) D.audit(req.user, 'shift_hours', `${u.name} -> ${(req.body || {}).hours}h`, req.ip);
  res.status(r.ok ? 200 : 400).json({ ...r, users: D.allUsers(req.scope) });
});

app.post('/api/admin/shift-hours/bulk', requireUser, requireAdmin, (req, res) => {
  const b = req.body || {};
  const r = D.setShiftHoursBulk(b.department || null, b.hours, req.scope);
  if (r.ok) D.audit(req.user, 'shift_hours_bulk', `${b.department || 'tất cả'} ${b.hours}h`, req.ip);
  res.status(r.ok ? 200 : 400).json({ ...r, users: D.allUsers(req.scope) });
});
...
/* Soi sheet: hệ thống đọc được mã nào, ngày nào — để tìm chỗ lệch */
app.get('/api/admin/reports/inspect', requireUser, requireAdmin, async (req, res) => {
  const day = String(req.query.day || '').match(/^\d{4}-\d{2}-\d{2}$/)
    ? req.query.day : new Date().toISOString().slice(0, 10);
  res.json(await Sheets.inspectDay(day, req.query.refresh === '1'));
});
...
app.get('/api/late', requireUser, (req, res) => {
  ...
  res.json({
    detail,
    shifts: D.shiftOptions(scope === null ? null : scope),
    ...(detail
...
app.post('/api/admin/schedule/import',
  ...
  (req, res) => {
    ...
    let parsed;
    try {
      // Truyền độ dài ca của từng người vào, để ô chỉ ghi 1 mốc giờ
      // tính được giờ kết đúng theo người đó (8 tiếng hay 10 tiếng).
      const hoursOf = (empCode, name) => {
        const u = (empCode && D.db.prepare('SELECT shift_hours FROM users WHERE emp_code=?').get(String(empCode).toUpperCase()))
               || (name && D.db.prepare('SELECT shift_hours FROM users WHERE name=?').get(name));
        return u ? Number(u.shift_hours) || 8 : 8;
      };
      parsed = parseScheduleFile(req.body, ym, hoursOf);
    } catch (e) {
    ...
    if (mode === 'preview') {
      ...
      return res.json({
        ...
        warnings: D.checkScheduleRows(parsed.rows),
